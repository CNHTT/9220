...
public class LoginActivity extends BaseActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.clean_password:
                etPassword.setText("");
                break;
            case R.id.iv_show_pwd: if (etPassword.getInputType() != InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD) {
    ...
    private void login() {
        ...
        sn = etMobile.getText().toString();
        password = etPassword.getText().toString();

        if (DataUtils.isNullString(sn)||DataUtils.isNullString(password))
        {
            ToastUtils.showToast("Please input sn/password");return;
        }

        showProgressDialog(R.string.xw_ptr_loading);
        PostResult postResult = new PostResult();
        postResult.setSn(sn);
        postResult.setPassword(password);

        new HttpBuilder(AppUrl.url_login)
    ...
    private void toLogin(String s) {
        LoginBean loginBean = (LoginBean) JsonUtil.stringToObject(s,LoginBean.class);
        if (loginBean.getRst()){
            SPUtils.putString(this, AppUrl.value_token,loginBean.getToken());
            SPUtils.putString(this, AppUrl.value_sn,loginBean.getSn());
            SPUtils.putBoolean(this,AppUrl.value_boolean,true);
            startActivity(new Intent(this,MainActivity.class));
            AppManager.getAppManager().finishActivity(this);

        }else {
            showDialogSuccessToast(loginBean.getDetail());
        }
    }
}
