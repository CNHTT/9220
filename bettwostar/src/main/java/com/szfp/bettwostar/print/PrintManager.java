...
//    }
    ...
    public int printWeek(LoginBean resultBean) {
        ...
        canvas.drawText("Week Result" , paint);
        ...
        canvas.drawText(" " , paint);
        setFontStyle(paint , 2 , true);
        canvas.drawText( "Week :"+ resultBean.getData().getWeek(), paint);
        canvas.drawText( "Drawn:"+ resultBean.getData().getDrawn(), paint);
        printLine(paint,canvas);
        ret = printData(canvas);
        return ret ;
    }

    public int printWinList(LoginBean resultBean) {
        this.printTask = new PrintTask();
        this.printFlag = true;
        int ret ;
        printer = Printer.getInstance() ;
        ...
        canvas.drawText(" " , paint);
        setFontStyle(paint , 2 , true);
        canvas.drawText( "Agent :"+ resultBean.getData().getAgent(), paint);
        canvas.drawText( "Operator :"+ resultBean.getData().getOperator(), paint);
        canvas.drawText( "Close Date  :"+resultBean.getData().getClose_date(), paint);
        canvas.drawText( "Total   :"+resultBean.getData().getTotal(), paint);
        canvas.drawText( "WinList   :"+resultBean.getData().getWinlist().toString(), paint);
    ...
    public int printReport(LoginBean resultBean) {
        this.printTask = new PrintTask();
        this.printFlag = true;
        int ret ;
        printer = Printer.getInstance() ;
        ...
        canvas.drawText(" " , paint);
        setFontStyle(paint , 2 , true);
        canvas.drawText( "total_payable :"+ resultBean.getData().getTotal_payable(), paint);
        canvas.drawText( "total_winning:"+ resultBean.getData().getTotal_winning(), paint);
        canvas.drawText( "balance_comp:"+resultBean.getData().getBalance_comp(), paint);
        canvas.drawText( "balance_agent   :"+resultBean.getData().getBalance_agen(), paint);
        printLine(paint,canvas);
        List<OddSummary> list = resultBean.getData().getOdd_summary();
        if (!DataUtils.isEmpty(list)){
            for (int i = 0; i <list.size() ; i++) {
                OddSummary data = list.get(i);
                canvas.drawText( i+1+"  " +sorts.get( Integer.valueOf(data.getSort())-1) +
                        "     Count"  +  data.getCount() , paint);
                canvas.drawText( "  Gross: " +  data.getGross()
                        + "  Payable: "+data.getPayable() + "  Commission: "+data.getCommission(), paint);
            }
        }
        printLine(paint,canvas);
    ...
    public  int printPosRecord(LoginBean.DataBean posRecord) {
        ...
        for (int i = 0; i <list.size() ; i++) {
            item = list.get(i);
            if (item.getStatus().equals("1"))continue;
            canvas.drawText(" " , paint);
            switch (item.getType()){
                case "1":
    ...
    public  int printPosRecord1(LoginBean.DataBean posRecord) {
        this.printTask = new PrintTask();
        this.printFlag = true;
        int ret ;
        printer = Printer.getInstance() ;
